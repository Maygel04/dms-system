...
<a href="#"
class="btn btn-sm btn-primary openFile"
data-file="{{ asset('uploads/'.$f->file_name) }}"
data-id="{{ $f->id }}">
View
</a>
...
@if($hasRemark)
<div class="card shadow-sm mb-4">
<div class="card-header bg-warning">📝 BFP Latest Remark</div>
<div class="card-body">
...
{{-- FINAL VERIFY --}}
<form method="POST" action="{{ route('bfp.verify', $application->id) }}" class="mt-3">
@csrf
...
{{-- REMARK --}}
<form method="POST" action="{{ route('bfp.saveRemark') }}" class="mt-3">
@csrf
<input type="hidden" name="application_id" value="{{ $application->id }}">
