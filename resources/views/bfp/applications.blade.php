...
<td>
    ...
    @elseif($a->bfp_status == 'assessed')

        <span class="badge bg-info">
            ASSESSED
        </span>

    @elseif($a->bfp_status == 'verified')

        <span class="badge bg-success">
            VERIFIED
        </span>

    @elseif($a->bfp_status == 'pending')
...
<td>
<a href="{{ url('/bfp/applications?app_id='.$a->id) }}"
...
<td>
    <button type="button"
        ...
        data-file="{{ asset('storage/bfp_docs/'.$f->file_name) }}">
        👁 View File
    </button>
</td>
...
<form method="POST" action="{{ route('bfp.saveRemark') }}">
...
<a href="{{ url('/bfp/receipt/'.$application->id) }}"
...
<form method="POST" action="{{ route('bfp.issue') }}">
@csrf
<input type="hidden" name="application_id" value="{{ $application->id }}">
...
<form method="POST" action="{{ route('bfp.mark.paid') }}">
@csrf
<input type="hidden" name="application_id" value="{{ $application->id }}">
...
<form method="POST" action="{{ route('bfp.saveAssessment') }}">
@csrf
...
<form method="POST" action="{{ route('bfp.verify',$application->id) }}">
@csrf
