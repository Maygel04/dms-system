...
@foreach($mpdoGrouped as $group)
    @foreach($group->sortBy('created_at')->values() as $index => $d)
        <tr>
            ...
            <td class="file-name">
                ...
                @else
                    <span class="badge bg-warning text-dark mt-1">RE-UPLOAD {{ $index }}</span>
                @endif
            </td>

            <td>
                {{ \Carbon\Carbon::parse($d->created_at)->format('Y-m-d H:i') }}
            </td>

            <td>
                <button class="btn btn-sm btn-success openFile"
                    data-file="{{ asset('storage/'.$d->file_path) }}">
                    View
                </button>
            </td>
        </tr>
